...
A mouse-only cursor pairs a precise dot with a trailing `rr` monogram, inspired by the read-only references. While visible, it cycles between sans, italic and outline lettering every 600 ms, including during scrolling. Hover feedback follows the element beneath the pointer as the page scrolls. It uses no additional dependencies and stops requesting movement frames when settled; the variant timer stops when hidden. Touch, reduced-motion preferences and keyboard navigation retain the native cursor. Other hover accents animate only on interaction.
